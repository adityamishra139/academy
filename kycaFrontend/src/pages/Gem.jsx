...
const Gem = () => {
  // Define a state to hold the gems data
  const [gems, setGems] = useState([]);
  const [loading, setLoading] = useState(true);

  // Fetch gems from the API when the component mounts
  useEffect(() => {
    const fetchGems = async () => {
      try {
        const response = await axios.get("http://localhost:3000/api/gems"); // Replace with your actual API URL
        setGems(response.data); // Set the fetched gems data
